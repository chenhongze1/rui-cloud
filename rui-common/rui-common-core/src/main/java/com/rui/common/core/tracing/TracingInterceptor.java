...
public class TracingInterceptor implements HandlerInterceptor {
    ...
    private void setSpanStatusFromHttpCode(Span span, int httpStatus) {
        if (httpStatus >= 200 && httpStatus < 400) {
            tracingManager.setSpanStatus(span, StatusCode.OK, "HTTP " + httpStatus);
        } else if (httpStatus >= 400 && httpStatus < 500) {
            tracingManager.setSpanStatus(span, StatusCode.ERROR, "HTTP " + httpStatus + " Client Error");
        } else if (httpStatus >= 500) {
            tracingManager.setSpanStatus(span, StatusCode.ERROR, "HTTP " + httpStatus + " Server Error");
        } else {
            tracingManager.setSpanStatus(span, StatusCode.UNSET, "HTTP " + httpStatus);
        }
    }
}