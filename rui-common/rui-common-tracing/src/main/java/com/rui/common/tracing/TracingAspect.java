...
public class TracingAspect {
            ...
            if (parameters.length > 0 && args.length > 0) {
                IntStream.range(0, Math.min(parameters.length, args.length))
                    .forEach(i -> {
                        ...
                        if (paramValue != null) {
                            String sanitizedValue = sanitizeParameterValue(paramValue);
                            span.setAttribute("param." + paramName, sanitizedValue);
                        }
                    });
            }
        } catch (Exception e) {
            log.warn("Failed to record parameters for span: {}", e.getMessage());
        }
    }
    ...
    private void recordReturnValue(Span span, Object result) {
        try {
            String sanitizedValue = sanitizeParameterValue(result);
            span.setAttribute("return.value", sanitizedValue);
            span.setAttribute("return.type", result.getClass().getSimpleName());
        } catch (Exception e) {
            log.warn("Failed to record return value for span: {}", e.getMessage());
        }
    }
